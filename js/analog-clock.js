    ...
    var startClock = function (_options) {
            ...
            var _option_timezone = _options.timezone || _local_timezone; // local timezone as fallback

            // Containers for Time Display 
                // Timezone Name
            var _option_cityDisplay = (_options.displayPrint && _options.displayPrint.city) ? _options.displayPrint.city : null;
                // hour
            var _option_hourDisplay = (_options.displayPrint && _options.displayPrint.hour) ? _options.displayPrint.hour : null;
                // minute
            var _option_minuteDisplay = (_options.displayPrint && _options.displayPrint.minute) ? _options.displayPrint.minute : null;
                // second
            var _option_secondDisplay = (_options.displayPrint && _options.displayPrint.second) ? _options.displayPrint.second : null;
                // day
            var _option_dayDisplay = (_options.displayPrint && _options.displayPrint.day) ? _options.displayPrint.day : null;
                // date
            var _option_dateDisplay = (_options.displayPrint && _options.displayPrint.date) ? _options.displayPrint.date : null;
                // month
            var _option_monthDisplay = (_options.displayPrint && _options.displayPrint.month) ? _options.displayPrint.month : null;
                // year
            var _option_yearDisplay = (_options.displayPrint && _options.displayPrint.year) ? _options.displayPrint.year : null;

            // Containers for Action Hands
                // hour hand
            var _option_hourHand = (_options.actionHand && _options.actionHand.hour) ? _options.actionHand.hour : null;
                // minute hand
            var _option_minuteHand = (_options.actionHand && _options.actionHand.minute) ? _options.actionHand.minute : null;
                // second hand
            var _option_secondHand = (_options.actionHand && _options.actionHand.second) ? _options.actionHand.second : null;

            // Timezones with their names
            // Each should be identical
            // valid timezone inputs
            var _option_valid_zones = (_options.timeZones && _options.timeZones.length!=0) ? _options.timeZones : null;
            ...
            if (_option_valid_zones) {
                for (var i = 0; i < _option_valid_zones.length; i++) {
            ...
            var _new_correct_time = new Date(_local_time_london + (Number(_option_timezone) * 60 * 60 * 1000));
            ...
            // Prepare to print/apply   
                // hands rotation
            var _second_rotation = secondsFraction*360;
            var _minute_rotation = minutesFraction*360;
            var _hour_rotation = hoursFraction*360;
                // date string
            var _print_date = _new_correct_date;
            var _print_day = _days[_new_correct_day];
            var _print_month = _months[_new_correct_month];
            var _print_year = _new_correct_year;
                // timezone name
            var _print_city = _whichCity;
                // date time
            var _print_second = _new_correct_second;
            var _print_minute = _new_correct_minute;
            var _print_hour = _new_correct_hour;
...
var _selectCity = document.querySelector('#selectCity');
if (_selectCity) {
    var myDateLoco = -(new Date().getTimezoneOffset()) / 60;
    myDateLoco = Math.floor(myDateLoco);
    for (var i = 0; i < _some_time_zones.length; i++) {
        var _addin1 = ' ';
